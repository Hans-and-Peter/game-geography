...
list_task_cmd = "aws ecs list-tasks --service-name \"#{service_name}\" " +
...
describe_instance_cmd = "aws ecs describe-container-instances --container-instances \"#{task_container_id}\" " +
    "--output text --query 'containerInstances[*].ec2InstanceId'"
task_instance_id = shell(describe_instance_cmd)

# get public ip of container instance
describe_instance_cmd = "aws ec2 describe-instances --instance-id \"#{task_instance_id}\" "+
    "--output text --query 'Reservations[*].Instances[*].NetworkInterfaces[*].Association.PublicDnsName'"
public_host = shell(describe_instance_cmd)
